...
class AnalyzeByXPath(doc: Any) {
    ...
    private fun getResult(xPath: String): List<JXNode>? {
        val node = jxNode
        return if (node is JXNode) node.sel(xPath) else (node as JXDocument).selN(xPath)
    }
        ...
        if (results.isNotEmpty()) {
            if ("%%" == ruleAnalyzes.elementsType) {
                for (i in results[0].indices) {
                    for (temp in results) { if (i < temp.size) jxNodes.add(temp[i]) }
                }
            } else {
                for (temp in results) jxNodes.addAll(temp)
            }
        }
        return jxNodes
    }
    ...
    internal fun getStringList(xPath: String): List<String> {
        ...
        val rules = ruleAnalyzes.splitRule("&&", "||", "%%")
        if (rules.size == 1) {
            getResult(xPath)?.map { result.add(it.asString()) }
            return result
        }
    ...
    fun getString(rule: String): String? {
        ...
        val rules = ruleAnalyzes.splitRule("&&", "||")
        if (rules.size == 1) {
            getResult(rule)?.let { return it.joinToString("\n") { n -> n.asString() } }
            return null
        }
